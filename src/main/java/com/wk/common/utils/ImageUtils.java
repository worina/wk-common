...
public class ImageUtils {
	 ...
	 * 几种常见的图片格式
	 */
	public static String GIF = "GIF";// 图形交换格式
	public static String JPG = "JPG";// 联合照片专家组
	public static String JPEG = "JPEG";// 联合照片专家组
	public static String BMP = "BMP";// 英文Bitmap（位图）的简写，它是Windows操作系统中的标准图像文件格式
	public static String PNG = "PNG";// 可移植网络图形
	 ...
	 *            缩放后的宽度
	 * @param fillColor
	 *            比例不对时填充的颜色，不填充填写null，填充使用java.awt.Color中包含的色;
	 */
	public static void scale2(String srcImageFile, String result, int height, int width, Color fillColor) {
		try {
			...
			if (null != fillColor) {// 补白
				BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
				Graphics2D g = image.createGraphics();
				g.setColor(fillColor);
				g.fillRect(0, 0, width, height);
				if (width == itemp.getWidth(null))
					g.drawImage(itemp, 0, (height - itemp.getHeight(null)) / 2, itemp.getWidth(null), itemp.getHeight(null), fillColor, null);
				else
					g.drawImage(itemp, (width - itemp.getWidth(null)) / 2, 0, itemp.getWidth(null), itemp.getHeight(null), fillColor, null);
				g.dispose();
				itemp = image;
			}
